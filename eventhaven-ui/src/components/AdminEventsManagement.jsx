...
const AdminEventsManagement = memo(({ onToast }) => {
  ...
  const load = async () => {
    setLoading(true);
    try { 
      const r = await api.get('/events?size=100&includeDraft=true'); // Fetch enough for client-side pagination or use server-side
  ...
  const handleEdit = async (event) => {
    try {
      const response = await api.get(`/events/${event.id}?includeDraft=true`);
      setEditTarget(response.data?.data || response.data || event);
    } catch {
      setEditTarget(event);
      onToast?.('Unable to load full event details. Opening the latest list snapshot.', 'warning');
    }
    setShowForm(true);
  };
                    ...
                    <tr key={ev.id} className="hover:bg-gray-50/60 transition">
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-1">
                          ...
                          <button title="Edit" onClick={()=>handleEdit(ev)}
                            className="w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-500 hover:text-violet-600 hover:border-violet-300 hover:bg-violet-50 transition">
